...
function Converter() {
  ...
  const handleCharFocus = (
    ...
    setFocusU16(!isFocus);
    !charValue && setU16Value("");
  };
  const handleU16Focus = (
    e: React.ChangeEvent<HTMLTextAreaElement>,
    isFocus: boolean
  ): void => {
    let inputValue = e.target.value;
  ...
  const convertChar2Utf16 = (str: string) => {
    ...
    const u16Arr = strArr.map((string) => {
      ...
      let u16Body: (string | number | undefined)[] = [];
      if (codeArr.length <= 1) {
        let codePt = codeArr[0];
        u16Body.push(codePt);
      ...
      switch (output) {
        ...
        case "htmlEn":
          ...
          break;
        default:
          const arr = u16Body.map((item) => {
            if (typeof item === "number") {
              let hexVal = helpers.dec2hex(item as number);
  ...
  const convertUtf162Char = (u16: string) => {
    ...
    const charArr = u16Arr.map((unifiedValue) => {
      ...
      let hexValue: number | string = parseInt(value, 16);
      let codePtStr;
      switch (output) {
        case "codePt":
          codePtStr = value.slice(1, value.length - 1);
          const codePtInt = parseInt(codePtStr, 16);
          if (helpers.isInRange(codePtInt))
            return String.fromCodePoint(codePtInt);
          break;
        case "htmlEn":
  ...
  const getU16Value = () => {
    return charValue ? convertChar2Utf16(charValue) : u16Value;
  };
  const getCharValue = () => {
    return u16Value ? convertUtf162Char(u16Value) : charValue;
  };
  ...
  const getEncodeType = () => {
    switch (output) {
      case "utf16":
        ...
        break;
      case "codePt":
        return `${t("code-point")} (Code point)`;
        break;
      case "htmlEn":
  ...
  const handleCopyChar = () => {
    copy(getCharValue() as string);
    setHasCopiedChar(true);
  };

  const handleCopyU16 = () => {
    copy(getU16Value() as string);
  ...
  return (
    <Box px={[2, 4, 12]} mt={4} mb={10}>
      ...
      >
        ...
        <Spacer />
        <Button onClick={handleCopyChar} m={0}>
      ...
      >
        ...
        <Spacer />
        <Button onClick={handleCopyU16} ml={2}>
      ...
      <Stack direction="row" mt={4}>
        ...
        <Spacer />
        <Button colorScheme="teal" size="sm" onClick={handleClear}>
          {t("clear")}
        </Button>
      </Stack>
